...
#1. transform each sentence to START WORD_INDEX1 WORD_INDEX2 ... END
#2. Form a vocabulatory
import itertools
import itertools
import operator
import numpy as np
...
def load_data(filename="data/reddit-comments-2015-08.csv", vocabulary_size=2000, min_sent_characters=0):
    ...
    with open(filename,'rb') as f:
        line = f.readlines()
        sentences = itertools.chain(*[nltk.sent_tokenize(x.decode('utf-8').lower()) for x in line])
...
def print_sentence(s, index_to_word, MODEL_FILE_NAME=None):
    sentence_str = [index_to_word[x] for x in s[1:-1]]
    try:
        sentenc = " ".join(sentence_str)
        print(sentenc)
        # Write to file

        if MODEL_FILE_NAME != None:
            with open("{0}.txt".format(MODEL_FILE_NAME), "a+") as file:
                file.write(sentenc + "\n")
...
def generate_sentence(model, index_to_word, word_to_index, min_length=20):
...
def generate_sentences(model, n, index_to_word, word_to_index, MODEL_FILE_NAME=None):
    for i in range(n):
        sent = None
        while not sent:
            sent = generate_sentence(model, index_to_word, word_to_index)
        print_sentence(sent, index_to_word, MODEL_FILE_NAME)
